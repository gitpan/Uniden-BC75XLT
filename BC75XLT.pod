...
B<fatal> - boolean, if set to true all errors are supposed as fatal and C<die>
...
B<parity>, B<stopbits>, B<handshake> and B<read_cost_time> you can read in 
...
=head2 OBJ->command(CMD_STR, ARGS_ARRAYREF)
...
B<status> - string, B<OK> - if command executed successfuly and B<ERROR> if 
something was wrong
...
needs PGM, it will enter to PRG before send command and exit after. So if you
want to send sequence of commands you may start PRM manually and exit it after
sequence is complite. It may save you some seconds.
...
Returns ARRAYREF of 10 elements. Each element contains ON/OFF string. 
...
If C<GROUP_DATA> is HASHREF then keys mean to be index of radion scanner 
group and values are states of group. States can be strings - ON/OFF or integer
1:OFF/0:ON.

If C<GROUP_DATA>  is ARRAYREF then every element is state of group with index
...
Set valid radio scanner groups in Scan mode. C<CHANNELS_ARRAYREF> is ARRAYREF
...
Get saved channel information, С<N_INT> is number of channel, integer 
between 1..300.
...
B<freq> - saved freqency in human readable form, e.g. 128.0000

=item *

B<freq_code> - saved frequency as returned by radio scanner, e.g. '01280000'
...
B<start> - number of first channel

=item *

B<stop> - number of last channels
...
Returns ARRAYREF each element is HASHREF with information about channel
described in method C<getChannelInfo>.

=head2 getBankChannelsInfo(N_INT)

Gets information about channel in a bank. C<N_INT> is number of channels
bank, 1..10.
...
Store frequency in to channel. C<IDX_INT> - integer, number of channel (1..300).

C<DATA_HASHREF> is HASHREF, information about stored frequency with keys:
...
Erase information stored in channel. C<IDX_INT> = integer, number of channel.
...
C<OPTS_HASHREF> is HASHREF with keys:
...
B<VHF_LOW>, B<AIR>, B<VHF_HIGH> and B<UHF>. They are keys in B<bands> HASHREF,
...
B<OPTS_HASHREF> is HASHREF with keys:
...
B<bands> - list of available bands in Close Call RF mode. ARRAYREF each element
is band name to be set in ON state.
...
Returns information about Service Search settings. C<BAND_INT> is number or
name of one of 10 Service frequences ranges. 
...
Returns state of custom seach frequencies groups and Custom Search settings.
Return value is HAHREF with keys:

=over 3

=item *

B<group> -  ARRAYREF of 10 elements. Each element contains ON/OFF string.
Every array element describes group with the same index in radio.
But index of radio scanner groups starts with 1. So element with index 0 means
state of group with index 1.
...
C<GROUPS> - HASHREF or ARRAYREF - describes states of freq group. 
If you use ARRAYREF you should set 10 elements array every element must 
contain string ON or OFF. In HASHREF representation keys are index of groups, 
values are state. If group state was not set in HASH its state would be set 
to OFF.
...
Radio scanner has 10 Custom Search frequency ranges. This method retrive 
...
Returns all ten ranges bounds as ARRAYREF every element contains 
two elements ARRAYREF where first element is left bound and second right.
